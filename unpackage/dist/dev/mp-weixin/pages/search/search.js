...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onUnload(() => {
      common_vendor.index.__f__("log", "at pages/search/search.vue:130", "remove");
      common_vendor.index.removeStorageSync("picList");
    });
    const searchWall = async () => {
      try {
        let result = await api_apis.apiSearchWall(queryParams.value);
        classList.value = [...classList.value, ...result.data];
        common_vendor.index.setStorageSync("picList", classList.value);
        if (queryParams.value.pageSize != result.data.length)
          noData.value = true;
        if (classList.value.length == 0 && result.data.length == 0)
          noSearch.value = true;
        common_vendor.index.__f__("log", "at pages/search/search.vue:145", result);
      } finally {
